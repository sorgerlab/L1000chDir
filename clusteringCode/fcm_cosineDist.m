function [center, U, obj_fcn, logstr] = fcm_cosineDist(data, cluster_n, options)
...
% normalization of the data (for cosine)
data = data ./ (sqrt(sum(data.^2,2))*ones(1,size(data,2)));
...
for i = 1:max_iter,
    ...
    if i > 1
        if abs(obj_fcn(i) - obj_fcn(i-1)) < min_impro
            logstr = sprintf('Loop done in %.2f ', toc(t));
            break; end,
        if toc(t)>(max_iter/2)
            logstr = sprintf('Loop in fcm_cosineDist spent too much time(%.2f); ', toc(t));
            break;end
    end
...
logstr = [logstr 'at it=' num2str(iter_n)];
...
center = center ./ (sqrt(sum(center.^2,2))*ones(1,size(center,2))); % normalization for cosine
